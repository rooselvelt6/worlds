    ...
    for (const entry of batch) {
        ...
        if (isVeg) {
            ...
            mat = new THREE.MeshStandardMaterial({
                ...
                map: hasUVs ? textureAtlas : null,
                normalMap: hasUVs && atlasUD ? atlasUD.normalMap : null,
                roughnessMap: hasUVs && atlasUD ? atlasUD.roughnessMap : null,
                metalnessMap: hasUVs && atlasUD ? atlasUD.metalnessMap : null,
...
window.threeBridgeSetMeshTransform = function (key, px, py, pz, rx, ry, rz, sx, sy, sz) {
    const mesh = meshes.get(key);
    if (!mesh) return;
...
let headlampLight = null;
window.threeBridgeSetHeadlamp = function (active, x, y, z, tx, ty, tz, intensity) {
    if (!scene) return;
    if (!headlampLight) {
        headlampLight = new THREE.SpotLight(0xffffff, 0);
        headlampLight.angle = 0.35;
        headlampLight.penumbra = 0.5;
        headlampLight.decay = 1;
        headlampLight.distance = 30;
        headlampLight.target.position.set(0, 0, -5);
        headlampLight.add(headlampLight.target);
        scene.add(headlampLight);
    }
    headlampLight.position.set(x, y, z);
    headlampLight.target.position.set(tx, ty, tz);
    headlampLight.intensity = intensity;
    headlampLight.visible = active && intensity > 0;
};

window.threeBridgeSetShadowQuality = function (resolution, near, far, left, right, top, bottom) {
    if (!csm) return;
    for (const light of csm.lights) {
...
let underwaterSceneFog = null;
window.threeBridgeSetUnderwaterFog = function (active, r, g, b, density) {
    if (!scene) return;
    if (active) {
        if (!underwaterSceneFog) {
            underwaterSceneFog = new THREE.FogExp2(new THREE.Color(r, g, b), density);
        } else {
            underwaterSceneFog.color.setRGB(r, g, b);
            underwaterSceneFog.density = density;
        }
        scene.fog = underwaterSceneFog;
    } else {
        scene.fog = null;
    }
};
