...
use crate::state::{ParticleMode, WorldParams};
...
pub fn mode_count(mode: ParticleMode) -> usize {
    match mode {
        ...
        ParticleMode::Snow => 600,
        _ => 0,
    }
}

pub fn mode_color_size(mode: ParticleMode) -> (f64, f64, f64, f64) {
    match mode {
        ParticleMode::Rain => (0.65, 0.75, 0.95, 0.05),
        ParticleMode::Snow => (0.95, 0.97, 1.0, 0.1),
        _ => (1.0, 1.0, 1.0, 0.1),
    }
}
...
impl ParticleSystem {
    ...
    fn reset_mode_particle(&mut self, i: usize, mode: ParticleMode, params: &WorldParams, px: f64, pz: f64) {
        let mut rng: u64 = (params.seed as u64).wrapping_mul(6364136223846793005)
            .wrapping_add(i as u64 * 374761393);
        let i3 = i * 3;
        let spread = 18.0;
        match mode {
            ParticleMode::Rain => {
                self.positions[i3] = px + (simple_rng(&mut rng) - 0.5) * spread * 2.0;
                self.positions[i3 + 1] = 15.0 + simple_rng(&mut rng) * 12.0;
                self.positions[i3 + 2] = pz + (simple_rng(&mut rng) - 0.5) * spread * 2.0;
                self.velocities[i] = [(simple_rng(&mut rng) - 0.5) * 0.2, -22.0 - simple_rng(&mut rng) * 6.0, (simple_rng(&mut rng) - 0.5) * 0.2];
                self.max_lifetime[i] = 0.8 + simple_rng(&mut rng) * 0.4;
                self.lifetimes[i] = self.max_lifetime[i];
            }
            ParticleMode::Snow => {
                self.positions[i3] = px + (simple_rng(&mut rng) - 0.5) * spread * 2.0;
                self.positions[i3 + 1] = 12.0 + simple_rng(&mut rng) * 10.0;
                self.positions[i3 + 2] = pz + (simple_rng(&mut rng) - 0.5) * spread * 2.0;
                self.velocities[i] = [(simple_rng(&mut rng) - 0.5) * 0.4, -0.8 - simple_rng(&mut rng) * 0.6, (simple_rng(&mut rng) - 0.5) * 0.4];
                self.max_lifetime[i] = 7.0 + simple_rng(&mut rng) * 5.0;
                self.lifetimes[i] = self.max_lifetime[i];
            }
            _ => {}
        }
    ...
    pub fn update(&mut self, delta: f64, zone: Zone, params: &WorldParams, px: f64, _py: f64, pz: f64, water_level: f64, mode: ParticleMode) {
        if mode != ParticleMode::Off {
            for i in 0..self.count {
                let i3 = i * 3;
                self.lifetimes[i] -= delta;
                if self.lifetimes[i] <= 0.0 {
                    self.reset_mode_particle(i, mode, params, px, pz);
                    continue;
                }
